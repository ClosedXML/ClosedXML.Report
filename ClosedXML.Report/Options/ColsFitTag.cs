namespace ClosedXML.Report.Options
{
    public class ColsFitTag : OptionTag
    {
        public override void Execute(ProcessingContext context)
        {
            ...
            // whole range
            else if (cellRow == xlRange.LastRow().RowNumber() && cellClmn == 1)
            {
                ws.Columns(xlRange.FirstColumnUsed().ColumnNumber(), xlRange.LastColumnUsed().ColumnNumber())
                    .AdjustToContents(xlRange.FirstRowUsed().RowNumber(), xlRange.LastRowUsed().RowNumber());
            }
            // range column
            if (cellRow == xlRange.FirstRow().RowNumber())
            {
