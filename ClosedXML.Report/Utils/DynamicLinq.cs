...
namespace ClosedXML.Report.Utils
{
    public static class DynamicQueryable
    {
        ...
        public static object Aggregate(this IQueryable source, string function, string member)
        {
            ...
            LambdaExpression selector = Expression.Lambda(Expression.MakeMemberAccess(parameter, property), parameter);
            // We've tried to find an expression of the type Expression<Func<TSource, TAcc>>,
            // which is expressed as ( (TSource s) => s.Price );

            return Aggregate(source, function, selector);
            /*// Method
        ...
        public static object Aggregate(this IQueryable source, string function, LambdaExpression selector)
        {
            if (source == null) throw new ArgumentNullException("source");
            if (selector == null) throw new ArgumentNullException("selector");
            ...
            // Method
            MethodInfo aggregateMethod = typeof(Queryable).GetMethods().SingleOrDefault(
                m => string.Equals(m.Name, function, StringComparison.InvariantCultureIgnoreCase)
                     && m.ReturnType == selector.ReturnType // should match the type of the property
                    && m.IsGenericMethod);
            ...
            else
            {
                ...
                return source.Provider.Execute(
                    Expression.Call(
                        null,
                        aggregateMethod.MakeGenericMethod(new[] { source.ElementType, selector.ReturnType }),
                        new[] { source.Expression, Expression.Quote(selector) }));
            }
        }
            ...
            if (token.id == TokenId.OpenParen)
            {
                if (instance != null && type != typeof(string))
                {
                    ...
                    if (enumerableType != null)
                    {
                        ...
                        if (FindGenericType(typeof(IQueryable<>), type) == null)
                        {
                            var methods = typeof(Queryable).GetMethods(BindingFlags.Static | BindingFlags.Public);
                            var qmi = methods.First(m => m.Name == "AsQueryable" && m.GetParameters().First().ParameterType.IsGenericType);
                            qmi = qmi.MakeGenericMethod(elementType);
                            instance = Expression.Call(qmi, new [] { instance });
                        }
